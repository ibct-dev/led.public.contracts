...
namespace eosiosystem {
   ...
   void validate_ibct_vesting( int64_t stake ) {
      const int64_t base_time = 1575126000; // 2019-12-01
      const int64_t max_claimable = 0; // put nebtc lock amount
      // const int64_t max_claimable = 100'000'000'0000ll;
      const int64_t claimable = int64_t(max_claimable * double(current_time_point().sec_since_epoch() - base_time) / (10*seconds_per_year) ); // modify year parts(10)

      check( max_claimable - claimable <= stake, "ibct can only claim their tokens over 10 years" );
   }
   ...
   void system_contract::update_voting_power( const name& voter, const asset& total_update )
   {
      ...
      auto last_stake = voter_itr->last_stake;
      {
         _gstate.total_stake_amount -= last_stake;
         _gstate.total_stake_amount += total_update.amount;
      }
      ...
      if( voter == "ibct"_n ) {
         validate_ibct_vesting( voter_itr->staked );
      }

      if( voter_itr->interiors.size() || voter_itr->proxy ) {
         update_votes( voter, voter_itr->proxy, voter_itr->interiors, false );
      }
   }
