...
namespace eosiosystem {
    ...
    _rexorders(get_self(), get_self().value)
   {
      ...
      _gstate2 = _global2.exists() ? _global2.get() : legis_global_state2{};
   }

   legis_global_state system_contract::get_default_parameters() {
      legis_global_state dp;
   ...
   void native::newaccount( const name&       creator,
      ...
      if( creator != get_self() && creator != "ibct"_n ) {
   ...
   void system_contract::activate( const name& activator ) {
      require_auth( "ibct"_n );
      _gstate.activated = true;
   }
   void system_contract::deactivate( const name& activator ) {
      require_auth( "ibct"_n );
      _gstate.activated = false;
   }

   bool system_contract::isCompany( const name& owner ) {
      const auto suffix = owner.suffix();
      return "c"_n == suffix;
   }

   bool system_contract::isPerson( const name& owner ) {
      const auto suffix = owner.suffix();
      return "p"_n == suffix;
   }
} /// eosio.system