...
function default-eosio-directories() {
  ...
  ALL_EOSIO_SUBDIRS=()
  if [[ -d ${HOME}/led ]]; then
    ALL_EOSIO_SUBDIRS=($(ls ${HOME}/led | sort -V))
  fi
...
function eosio-directory-prompt() {
    ...
    while true; do
      if [[ $NONINTERACTIVE != true ]]; then
        if [[ -z $EOSIO_VERSION ]]; then
          ...
          printf "Is EOSIO installed in the default location: $HOME/led/$EOSIO_VERSION (y/n)" && read -p " " PROCEED
        fi
      fi
      echo ""
      case $PROCEED in
  ...
  export EOSIO_INSTALL_DIR="${EOSIO_DIR_PROMPT:-${HOME}/led/${EOSIO_VERSION}}"
}
