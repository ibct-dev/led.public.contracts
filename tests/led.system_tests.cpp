...
try {
    ...
    BOOST_REQUIRE_EQUAL(success(), push_action_msig( N(alice.p), N(exec), mvo()
                                                        ("proposer",      "alice.p")
...
try {
    ...
    BOOST_REQUIRE_EQUAL(success(), push_action_msig( N(alice.p), N(propose), mvo()
                                                    ("proposer",      "alice.p")
    ...
    BOOST_REQUIRE_EQUAL(wasm_assert_msg("transaction authorization failed"),
                        push_action_msig( N(alice.p), N(exec), mvo()
                                            ("proposer",      "alice.p")
                                            ("proposal_name", "upgrade1")
                                            ("executer",      "alice.p")
                        )
