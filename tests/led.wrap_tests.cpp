...
// class legis_wrap_tester : public tester {
//    public:
//     legis_wrap_tester() {
//         create_accounts({N(led.msig), N(prod1), N(prod2), N(prod3), N(prod4), N(prod5), N(alice), N(bob), N(carol)});
//         produce_block();
...
//         base_tester::push_action(config::system_account_name, N(setpriv),
//                                  config::system_account_name, mutable_variant_object()("account", "led.wrap")("is_priv", 1));

//         auto system_private_key = get_private_key(config::system_account_name, "active");
//         set_code(N(led.wrap), contracts::wrap_wasm(), &system_private_key);
//         set_abi(N(led.wrap), contracts::wrap_abi().data(), &system_private_key);
...
//         const auto& accnt = control->db().get<account_object, by_name>(N(led.wrap));
//         abi_def abi;
//         BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
//         abi_ser.set_abi(abi, abi_serializer_max_time);

//         while (control->pending_block_state()->header.producer.to_string() == "led") {
//             produce_block();
//         }
//     }

//     void propose(name proposer, name proposal_name, vector<permission_level> requested_permissions, const transaction& trx) {
//         push_action(N(led.msig), N(propose), proposer, mvo()("proposer", proposer)("proposal_name", proposal_name)("requested", requested_permissions)("trx", trx));
//     }

//     void approve(name proposer, name proposal_name, name approver) {
//         push_action(N(led.msig), N(approve), approver, mvo()("proposer", proposer)("proposal_name", proposal_name)("level", permission_level{approver, config::active_name}));
//     }

//     void unapprove(name proposer, name proposal_name, name unapprover) {
//         push_action(N(led.msig), N(unapprove), unapprover, mvo()("proposer", proposer)("proposal_name", proposal_name)("level", permission_level{unapprover, config::active_name}));
//     }
...
// transaction legis_wrap_tester::wrap_exec(account_name executer, const transaction& trx, uint32_t expiration) {
//     fc::variants v;
//     v.push_back(fc::mutable_variant_object()("actor", executer)("permission", name{config::active_name}));
//     v.push_back(fc::mutable_variant_object()("actor", "led.wrap")("permission", name{config::active_name}));
//     auto act_obj = fc::mutable_variant_object()("account", "led.wrap")("name", "exec")("authorization", v)("data", fc::mutable_variant_object()("executer", executer)("trx", trx));
//     transaction trx2;
...
// transaction legis_wrap_tester::reqauth(account_name from, const vector<permission_level>& auths, uint32_t expiration) {
//     fc::variants v;
//     for (auto& level : auths) {
//         v.push_back(fc::mutable_variant_object()("actor", level.actor)("permission", level.permission));
//     }
//     auto act_obj = fc::mutable_variant_object()("account", name{config::system_account_name})("name", "reqauth")("authorization", v)("data", fc::mutable_variant_object()("from", from));
//     transaction trx;
...
// BOOST_AUTO_TEST_SUITE(legis_wrap_tests)

// BOOST_FIXTURE_TEST_CASE(wrap_exec_direct, legis_wrap_tester)
// try {
...
//       wrap_trx.actions.emplace_back( get_action( N(led.wrap), N(exec),
//                                                  {{N(alice), config::active_name}, {N(led.wrap), config::active_name}},
//                                                  mvo()
//                                                    ("executer", "alice")
//                                                    ("trx", trx)
//       ) );
...
//     BOOST_REQUIRE_EQUAL("led", name{trace->action_traces[0].act.account});
//     BOOST_REQUIRE_EQUAL("reqauth", name{trace->action_traces[0].act.name});
//     BOOST_REQUIRE_EQUAL(transaction_receipt::executed, trace->receipt->status);
// }
// FC_LOG_AND_RETHROW()

// BOOST_FIXTURE_TEST_CASE(wrap_with_msig, legis_wrap_tester)
// try {
//     auto trx = reqauth(N(bob), {permission_level{N(bob), config::active_name}});
...
//     push_action(N(led.msig), N(exec), N(alice), mvo()("proposer", "carol")("proposal_name", "first")("executer", "alice"));
...
//     BOOST_REQUIRE_EQUAL("led.wrap", name{traces[0]->action_traces[0].act.account});
...
//     BOOST_REQUIRE_EQUAL("led", name{traces[1]->action_traces[0].act.account});
...
// BOOST_FIXTURE_TEST_CASE(wrap_with_msig_unapprove, legis_wrap_tester)
// try {
//     auto trx = reqauth(N(bob), {permission_level{N(bob), config::active_name}});
//     auto wrap_trx = wrap_exec(N(alice), trx);
...
//     // The proposal should not have sufficient approvals to pass the authorization checks of led.wrap::exec.
//     BOOST_REQUIRE_EXCEPTION(push_action(N(led.msig), N(exec), N(alice), mvo()("proposer", "carol")("proposal_name", "first")("executer", "alice")), eosio_assert_message_exception,
//                             eosio_assert_message_is("transaction authorization failed"));
// }
// FC_LOG_AND_RETHROW()

// BOOST_FIXTURE_TEST_CASE(wrap_with_msig_producers_change, legis_wrap_tester)
// try {
...
//     // The proposal has four of the five requested approvals but they are not sufficient to satisfy the authorization checks of led.wrap::exec.
//     BOOST_REQUIRE_EXCEPTION(push_action(N(led.msig), N(exec), N(alice), mvo()("proposer", "carol")("proposal_name", "first")("executer", "alice")), eosio_assert_message_exception,
//                             eosio_assert_message_is("transaction authorization failed"));
